...
define(['dojo/_base/declare',
  ...
  /* global jimuConfig*/
  var MIN_MARGIN = 8, ICON_SIZE = 40, ICON_IMG_SIZE = 30, BUTTON_SIZE = 60,
      NORMAL_MODE = 1, MOBILE_MODE = 2;
  var wActivo = 0;
  /**
   * @exports themes/INGEMMETTheme/widgets/AnchorBarController/Widget
   */
  var clazz = declare([BaseWidget, PoolControllerMixin], {
    ...
    _getGroupPanelPosition: function(dockableItem, widgetBox){
      ...
      position.top = iconBox.y + MIN_MARGIN; //INGEMMET
      position.left = iconBox.x + 40; //INGEMMET
      position.zIndex = 150; //INGEMMET
      if (window.appInfo.isRunInMobile)//INGEMMET
      {
    ...
    _getOffPanelPosition: function(dockableItem, widgetBox){
      ...
      //position.top = iconBox.y - widgetBox.h - MIN_MARGIN;
      position.top = iconBox.y + iconBox.h - 29;//INGEMMET

      if (window.isRTL) {
        if(iconBox.x < widgetBox.w + MIN_MARGIN){
            //position.left = mapBox.w - widgetBox.w - MIN_MARGIN; 
            position.left = 0;//INGEMMET
        }else{
          position.left = iconBox.x;
        }
      } else {
    ...
    _onGroupNodeClick: function(dockableItem, groupItem){
      ...
      if (dockableItem.isOpen) {
        ...
        wActivo = dockableItem.config.id;
